...
output <- data.table(unit=character(), year=numeric(), p_area_mn = numeric(), p_area_sd=numeric(), p_area_tot_km2=numeric(),
                     ...
                     p_contig_edge_dens_km_km2 = numeric(), p_core_tot_km2=numeric(), p_core_num = numeric())

p_data <- data.table(unit=character(), year=numeric(), id=numeric(), area_km2=numeric(),
                     perim_km=numeric(), core_km2=numeric(), core_num = numeric())

# define global options
options_landscapemetrics(to_disk = T)

# define constant parameters----
contiguous_area_thr <- 0.7  # proportion of most contiguous area to include in analysis
p_delin_dir <- 4
edge_depth_for_core <- 4 # equivalent to buffer of 100m for a 25m raster
...
for (i in 1:length(years)) {
  for (j in 1:nrow(code_table)) {
    ...
    curr_out <- calculate_lsm(landscape = curr_raster,
                              what = c("lsm_p_area","lsm_p_perim","lsm_c_ed","lsm_l_ta","lsm_p_core","lsm_p_ncore"),
                              directions = p_delin_dir,
                              edge_depth = edge_depth_for_core,
                              progress = F) %>%
      as.data.table()
    ...
    curr_p_data$core <- curr_p_data$core/100
    curr_p_data <- curr_p_data[order(-area),]
    setnames(curr_p_data,c("area","perim","core","ncore"),c("area_km2","perim_km","core_km2","core_num"))
    ...
    new_output_row[1, `:=`(p_area_mn = mean(curr_p_data[,area_km2]),
                   ...
                   p_core_tot_km2 = sum(curr_p_data[,core_km2]),
                   p_core_num = sum(curr_p_data[,core_num]))]
...
# plot some units across years, where the patch area is color-coded
units2plot <- 3

print("plotting patches...")

for (j in 1:length(units2plot)) {
  for (i in 1:length(years)) {
    var <- paste0(code_table[units2plot[j],unit_in_filename],"Na",years[i],".tif")
    print(var)
    curr_raster <- raster(x = paste0("Data/natural_and_forested_landscape/20220607/",var))
    
    show_lsm(landscape = curr_raster, what = "lsm_p_area", class = "1", label_lsm = T, directions = p_delin_dir)
  }
}
...
for (j in 1:length(units2plot)) {
  for (i in 1:length(years)) {
    ...
    show_cores(landscape = curr_raster, class = "1", directions = p_delin_dir, edge_depth = edge_depth_for_core)
  }
}
