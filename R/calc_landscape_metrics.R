...
output <- data.table(unit=character(), year=numeric(), p_area_mn = numeric(), p_area_sd=numeric(), p_total_area_km2=numeric(),
                     p_number=numeric(), edge_density_km_km2=numeric(), p_density_n_km2=numeric(),unit_total_area_km2=numeric())
...
for (i in 1:length(years)) {
  for (j in 1:nrow(code_table)) {
    ...
    new_output_row <- data.table(unit=code_table[j,unit],
                                 ...
                                 p_area_mn = curr_out[metric=="area_mn",value]/100,
                                 p_area_sd=curr_out[metric=="area_sd",value]/100,
                                 p_total_area_km2=curr_out[metric=="ca",value]/100,
                                 p_number=curr_out[metric=="np",value],
                                 edge_density_km_km2=curr_out[metric=="ed",value]/10,
                                 p_density_n_km2=curr_out[metric=="pd",value],
